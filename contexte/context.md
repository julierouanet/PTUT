...
| created_by_id      | TEXT    | Nullable, ajoute via migration (UUID Keycloak du créateur ; NULL pour l'existant seed/import XLSX) |
| created_by_name    | TEXT    | Nullable, ajoute via migration (renseigné sur `POST /api/equipment` et `POST /api/equipment/import-csv`) |
...
| `equipment_documents` | :750 | Documents équipement (soft delete `deleted_at`). **[MODIFIÉ 2026-06-23]** colonne `issue_id TEXT` ajoutée (nullable), `equipment_id` rendu nullable (reconstruction de table idempotente) — un document peut désormais être rattaché à un incident sans équipement. Limitation connue : les documents archivés avant cette migration ont `issue_id IS NULL` (pas de backfill, aucune correspondance fiable) — restent visibles dans l'onglet Documents de l'équipement mais absents du badge/KPI/section "documents d'intervention" côté incident. **[MODIFIÉ 2026-07-02]** colonnes `annex_number INTEGER` et `annex_type_index INTEGER` ajoutées (nullable) — compteur d'annexe partagé avec `issue_photos` pour les pièces jointes `document_type='completion'`, posé à l'upload (`POST /api/issues/:id/documents`) après tamponnage PDF réussi. `NULL` = document hérité (uploadé avant cette feature ou tamponnage en échec), jamais numéroté/tamponné. |
| `issue_photos` | :770 | Photos d'incidents (max 5). **[MODIFIÉ 2026-07-02]** colonnes `annex_number`/`annex_type_index INTEGER` (nullable, compteur partagé avec `equipment_documents`) et `annex_pdf_stored_name TEXT` (nullable) — nom de fichier d'une copie PDF tamponnée générée à l'upload, l'image originale (`stored_name`) n'étant jamais modifiée. **[MODIFIÉ 2026-07-03]** colonnes `uploaded_by TEXT`/`uploader_name TEXT` ajoutées (nullable) — traçabilité de l'uploadeur (posées à l'upload dans `POST /api/issues/:id/photos`), alignées sur `equipment_documents`. `NULL` = photo uploadée avant cette migration (aucun backfill possible). Exposées dans `GET /:id/photos`, la vue combinée `GET /:id/documents` et l'export `/api/documents/interventions`. |
| `notifications` | :793 | Notifications in-app |
| `issue_intervention_sessions` | :~ (voir `PRAGMA table_info`) | Boucles d'intervention (1:N par incident, `loop_number`). **[MODIFIÉ 2026-07-03]** colonne `next_action_due_at TEXT` ajoutée (nullable) — échéance ISO 8601 de `next_actions`, saisie optionnelle par le technicien à la clôture d'une boucle non résolue (`POST /api/issues/:id/sessions/active/close`). |
...
| POST    | /api/equipment                  | Admin/Supervisor  | Creer (required: id, name, department, category) + optionnel: **subcategory_id**, **warranty_end_date**, **criticality**. **[MODIFIÉ 2026-07-06]** renseigne `created_by_id`/`created_by_name` depuis `req.user` |
| POST    | /api/equipment/import-csv       | Admin/Sup/Tech    | **[NOUVEAU 2026-07-06]** Import en masse depuis un fichier CSV (multipart, champ `file`, 5 Mo max). `?dry_run=true` : valide sans écrire (`{dry_run:true, would_insert, errors}`), sinon insère ligne par ligne (`{inserted, errors}`, pas de transaction globale). Colonnes requises : `name`/`department`/`category`/`serial_number` (en-tête). Id généré par slug (translittération accents + troncature 100 car., suffixe `-N` sur collision intra-fichier/DB). Doublon `serial_number` (fichier ou DB) → ligne rejetée (`reason: "équipement déjà existant (numéro de série en doublon)"`), pas les autres. Audit `create_equipment_import_csv` par ligne insérée (jamais en dry-run) |
...
| GET     | /api/equipment/:id/final-report | Admin/Sup/Tech    | **[NOUVEAU 2026-07-01]** Rapport final équipement : résumé consolidé + historique des interventions résolues (`issues.resolved_at IS NOT NULL`). Aucun filtre sur `equipment.status` (accessible même `Disposed`). Retourne `{equipment_id, equipment_name, equipment_tag, summary:{total_interventions, mttr_hours_avg, reopened_rate_pct, downtime_hours_total}, interventions:[{issue_id, resolved_at, technician_name, duration_hours, root_cause, summary, reopened}]}` trié `resolved_at DESC`. KPI en heures, arrondi 1 décimale ; `mttr_hours_avg`/`reopened_rate_pct` à `null` si aucune intervention. `downtime_hours_total` = somme des `duration_hours` des rapports `finalized` uniquement. Toujours 200 (jamais d'erreur sur scope vide). Lecture seule, pas de `logAction`. **[MODIFIÉ 2026-07-03]** `equipment_tag` (premier `tag_number` de `equipment_tags`, ordre alphabétique, `null` si aucun) — affiché dans l'en-tête du PDF `generateEquipmentFinalReport` côté Flutter |
| POST    | /api/equipment/restore          | Admin             | Restaurer equipement supprime                     |
| GET     | /api/equipment/:id/documents    | Auth (non-staff)  | Liste des documents actifs. Filtre optionnel: `?type=technical\|intervention\|certification\|completion\|final_report\|photo`. Retourne `[{id, document_type, original_name, mime_type, file_size_kb, uploader_name, uploaded_at, issue_id, issue_status, issue_created_at, annex_number, annex_type_index, kind}]` — `issue_*` nuls pour les documents sans incident associé (LEFT JOIN issues). **[MODIFIÉ 2026-07-03]** `UNION ALL` avec les photos (`issue_photos`, tamponnées uniquement) des incidents rattachés à l'équipement, exposées `document_type='photo'`/`kind='photo'` — téléchargement via `GET /api/issues/:issueId/photos/:photoId/download` |
...
| GET     | /api/equipment/:id/documents/:doc_id/download | Auth (non-staff) | **[NOUVEAU]** Téléchargement inline (`Content-Disposition: inline`). 404 si soft-deleted. **[MODIFIÉ 2026-07-03]** nom de téléchargement convivial `<Équipement> - <Tag> - Issue <id> - Annexe <n>` (`buildFriendlyDownloadName`, `db-service/src/utils/documents_repo.js`) — fichier physique (`stored_name`, UUID) jamais renommé, segments indisponibles omis |
...
| PATCH   | /api/issues/:id/link-equipment | Admin/Sup/Tech | Liaison tardive d'un incident créé sans équipement (`equipment_id IS NULL`) à un équipement du catalogue, **[MODIFIÉ 2026-07-09]** OU correction d'un équipement déjà lié (le signalant s'est trompé). Body: `{ equipment_id, reason? }` — `reason` (≥10 car.) **obligatoire uniquement** si un équipement est déjà lié côté serveur (source de vérité, pas un flag client). **400** si `equipment_id` manquant, ou si correction sans `reason`/`reason` trop court, ou si `equipment_id` égal à l'actuel (no-op) ; **409** si incident clôturé (`Completed`/`Rejected`) ou équipement `Disposed` ; **403** si technicien non-assigné/incident non `In Progress` (admin/supervisor exemptés) ; **404** si incident ou équipement introuvable. Effets : `equipment_id`/`equipment_name` posés, `equipment_linked_at` (ISO), ligne tracée appendée dans `actions` (distingue liaison initiale et correction, avec l'ancien équipement dans le libellé). Audit `link_issue_equipment` (`details` inclut `previous_equipment_id`/`reason` en cas de correction — pas de nouvelle colonne DB). |
...
| POST    | /api/issues/:id/photos      | Auth           | **[NOUVEAU]** Upload multipart (champ `photos`, max 5 fichiers JPEG/PNG, 5 Mo chacun). Vérifie que total ≤ 5. Retourne `{photos: [{id, original_name, file_size_kb}]}`. **[MODIFIÉ 2026-07-02]** Réserve un `annex_number`/`annex_type_index` (transaction, compteur partagé avec `equipment_documents`) puis génère une copie PDF tamponnée séparée (`annex_pdf_stored_name`, bandeau `Annexe {N} — Incident n° {issueId} — Photo {k} — Page 1/1`) — l'image originale (`stored_name`) n'est jamais modifiée. Échec non bloquant (garde-fou identique aux pièces jointes). **[MODIFIÉ 2026-07-03]** Enregistre aussi `uploaded_by`/`uploader_name` (`req.user`) |
| GET     | /api/issues/:id/photos      | Auth           | **[NOUVEAU]** Liste les photos de l'incident. Retourne `[{id, original_name, mime_type, file_size_kb, uploaded_at}]`. **[MODIFIÉ 2026-07-03]** ajoute `uploaded_by`/`uploader_name` |
| GET     | /api/issues/:id/photos/:photo_id/download | Auth | **[NOUVEAU]** Téléchargement inline d'une photo. `Content-Disposition: inline`. **[MODIFIÉ 2026-07-03]** nom de téléchargement convivial `<Équipement> - <Tag> - Issue <id> - Annexe <n>` (`buildFriendlyDownloadName`) — fichier physique jamais renommé |
...
| POST    | /api/issues/:id/sessions/active/close | Admin/Sup/Tech | **[NOUVEAU]** Ferme la session active. **400** si pas de session active, ou `resolved=false` sans `next_actions`. Calcule `duration_hours` via `julianday`. **[MODIFIÉ 2026-07-03]** Paramètre optionnel `next_action_due_at` (ISO 8601) — **400** si fourni et non parsable. Accepté même si `resolved=true`. Audit trail `close_intervention_session` (details enrichi de `next_action_due_at`) |
| GET     | /api/issues/:id/documents   | Auth           | **[NOUVEAU 2026-06-23]** Liste les documents PDF d'intervention de l'incident (`equipment_documents` filtré par `issue_id`). **[MODIFIÉ 2026-07-03]** `UNION ALL` avec `issue_photos` tamponnées (`annex_pdf_stored_name NOT NULL`), exposées sous `document_type='photo'`/`kind='photo'`, avec leur vrai `uploader_name` (colonne propre à `issue_photos` depuis cette date — `NULL` seulement pour les photos uploadées avant la migration). Retourne `[{id, document_type, original_name, mime_type, file_size_kb, uploader_name, uploaded_at, annex_number, annex_type_index, kind}]` — téléchargement des lignes `photo` via `GET /:id/photos/:photo_id/download` (pas `/documents/:doc_id/download`) |
| POST    | /api/issues/:id/documents   | Admin/Sup/Tech | **[NOUVEAU 2026-06-23]** Upload multipart multi-fichiers (champ `files`, max 5, PDF/JPEG/PNG 10 Mo chacun). Body `type` (défaut `completion`, whitelist incluant `intervention`). Rattache aussi à `equipment_id` si l'incident en a un. Audit trail `upload_intervention_document`. **[MODIFIÉ 2026-07-02]** Pour `type='completion'` uniquement : réserve un `annex_number`/`annex_type_index` (transaction, compteur partagé avec `issue_photos`) puis tamponne le PDF en place (bandeau permanent `Annexe {N} — Incident n° {issueId} — Pièce jointe {k} — Page {p}/{total}`, via `services/annex_stamp_service.js`) ; une image `completion` est convertie en PDF 1 page (extension du `stored_name` changée en `.pdf`). Écriture atomique (`.tmp` + `rename`). Échec de tamponnage (PDF corrompu/0 page) : non bloquant, `annex_number` reste `NULL` (document traité comme hérité) |
| GET     | /api/issues/:id/documents/:doc_id/download | Auth | **[NOUVEAU 2026-06-23]** Téléchargement inline d'un document d'intervention. `Content-Disposition: inline`. Audit trail `download_intervention_document`. **[MODIFIÉ 2026-07-03]** nom de téléchargement convivial `<Équipement> - <Tag> - Issue <id> - Annexe <n>` (`buildFriendlyDownloadName`) — fichier physique jamais renommé |
| DELETE  | /api/issues/:id/documents/:doc_id | Admin/Sup/Tech | **[NOUVEAU 2026-07-03]** Soft delete (`deleted_at`) d'un document d'intervention. Contrôle d'auteur : technicien ne peut supprimer que ses propres pièces jointes (`uploaded_by = req.user.id`) ; admin/supervisor peuvent supprimer n'importe lequel. **404** si doc introuvable ou déjà supprimé. **403** si non autorisé. Audit trail `delete_intervention_document`. Utilisé par le client Flutter pour l'annulation d'uploads en cours (race condition) et le nettoyage d'orphelins dans `dispose()`. |
...
`issue_photos`. **[MODIFIÉ 2026-07-03]** `issue_photos` porte désormais ses propres colonnes
`uploaded_by`/`uploader_name` (voir schéma ci-dessus) et alimente correctement `uploader_name` dans la
liste combinée (§ ci-dessous) ; en revanche `fetchFilteredPhotos` (utilisée par `/zip` et `/print-pdf`,
via `buildPhotoWhere`) ne filtre volontairement pas sur `uploaded_by` — les photos restent exclues de ces
deux exports si `?uploaded_by=` est renseigné, comportement inchangé.
...
| 2  | EquipmentListScreen      | viewEquipment           | SliverList virtualisé, tri sur 4 colonnes, filtres PM (retard/imminente), RBAC colonnes (staffMedical vs technicien), export CSV liste filtrée (18 colonnes dont sous-catégorie/macro-catégorie/créé le/créé par), bouton "Planifier PM" quick-action, délègue créa/édition à EquipmentFormScreen |
