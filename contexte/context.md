...
| GET     | /api/issues/:id/documents   | Auth           | **[NOUVEAU 2026-06-23]** Liste les documents PDF d'intervention de l'incident (`equipment_documents` filtré par `issue_id`). **[MODIFIÉ 2026-07-03]** `UNION ALL` avec `issue_photos` tamponnées (`annex_pdf_stored_name NOT NULL`), exposées sous `document_type='photo'`/`kind='photo'`, `uploader_name=NULL`. Retourne `[{id, document_type, original_name, mime_type, file_size_kb, uploader_name, uploaded_at, annex_number, annex_type_index, kind}]` — téléchargement des lignes `photo` via `GET /:id/photos/:photo_id/download` (pas `/documents/:doc_id/download`) |
